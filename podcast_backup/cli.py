...
def process_podcast(podcast: PodcastConfig, config: Config, debug: bool = False):
    ...
    feed, raw_xml = download_feed(podcast.podcast_url, debug=debug)
    ...
    feed_builder = FeedBuilder(
        feed, raw_xml, podcast.base_url, storage_dir, deleted_dir, podcast.podcast_url
    )
...
def _add_deleted_episodes_to_feed(
    feed_builder: FeedBuilder, metadata: dict, current_feed_urls: set
):
    ...
    for mp3_url, episode_data in metadata.items():
        ...
        feed_builder.add_deleted_episode(filename)
...
def main():
    ...
    parser.add_argument(
        "--config", "-c", type=str, help="Path to config file (default: config.toml)"
    )
    parser.add_argument(
    ...
    for podcast in config.podcasts:
        try:
            process_podcast(podcast, config, debug=args.debug)
